...
!     Use array q1 to init FFTS
  ...
  subroutine topo
    real(dp) :: x,y
    real(dp), parameter :: eta0 = 0.0d0, eps = 1.d0
    do ix=1,nx
       x = -pi + dble(ix-1)*hx
       do iy=1,ny
          y = -pi + dble(iy-1)*hy
  ...
  subroutine PVinit
    real(dp) :: x,y,rloc
    real(dp), parameter :: q10= 1.d0,q20=-1.d0,r10=0.5d0,r20=0.5d0
...
! ---- CASE 2: Merger layer 1 --- !          
          q1(ix,iy)  = q10*exp(-fac*(((x-r10)**2+y**2)/r10**2)**fac)
          q1(ix,iy)  = q1(ix,iy)+q10*exp(-fac*(((x+r10)**2+y**2)/r10**2)**fac)
          q2a(ix,iy) = 0.d0          
...
!    close(99)
  ...
  subroutine stepping_euler
    ...
    do kx=1,nx
       do ky=1,ny
          q1(kx,ky) = q1(kx,ky)/(1.d0+dfac*rks(kx,ky)) + dt*p1(kx,ky)
          q2(kx,ky) = q2(kx,ky)/(1.d0+dfac*rks(kx,ky)) + dt*p2(kx,ky)
       enddo
    enddo
